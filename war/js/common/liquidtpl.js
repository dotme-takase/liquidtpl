 ...
 */
$.initList = function ( listId, itemTplId, showEmptyMsg, url, params, callback ) {
        ...
        success: function(result){
            ...
            if(jsonData["result"] == "success"){
                if(callback){
                    ...
                    $.setListValues(listId, itemTplId, showEmptyMsg, jsonData)
                }
            } else {
...
$.setListValues = function( listId, itemTplId, showEmptyMsg, jsonData ){
    ...
    if(values && (values.length > 0)){
        ...
        var errorHtml = $("<div>").addClass("mt10").addClass("confirmMessage").text(jsonData["empty"]);
        $("#" + listId).replaceWith(errorHtml);
    } else {
        $("#" + listId).remove();
    }
    ...
    $.ajax({
        ...
        success: function(result){
            ...
            if(jsonData["result"] == "success"){
                ...
                var errorHtml = $("<div>").addClass("warningMessage").text(error);
...
$.setFormValues = function( formId, jsonData, beforeSubmit, afterSuccess ){
    ...
    for(key in sourceValues){
        var type = $("#" + formId).find("input[name='" + key + "']").attr("type");
                ...
                if(jsonData["result"] == "success"){
                    ...
                    if(jsonData["errors"]){
                        for(var key in jsonData["errors"]){
                            ...
                            .text(jsonData["errors"][key]).slideDown("fast");
                        }
                    } else {
                        $("#" + formId).find("#grobalError").text(error).slideDown("fast");
                    }
